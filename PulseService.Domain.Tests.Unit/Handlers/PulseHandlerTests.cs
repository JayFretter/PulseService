...
namespace PulseService.Domain.Tests.Unit.Handlers
{
    ...
    public class PulseHandlerTests
    {
        ...
        public async Task CreatePulseAsync_AddsPulse()
        {
            ...
            // Act
            await _pulseHandler.CreatePulseAsync(pulse);

            // Assert
            _mockPulseRepository.Verify(pr => pr.AddPulseAsync(pulse), Times.Once);
        }
        ...
        public async Task DeletePulseAsync_PulseExists_ReturnsTrue()
        {
            ...
            _mockPulseRepository.Setup(pr => pr.DeletePulseAsync(pulseId, userId)).ReturnsAsync(true);

            // Act
            var result = await _pulseHandler.DeletePulseAsync(pulseId, userId);
        ...
        public async Task DeletePulseAsync_PulseDoesNotExist_ReturnsFalse()
        {
            ...
            var pulseId = "pulse2";
            var userId = "user2";
            _mockPulseRepository.Setup(pr => pr.DeletePulseAsync(pulseId, userId)).ReturnsAsync(false);
        ...
        public async Task GetAllPulsesAsync_ReturnsListOfPulses()
        {
            ...
            _mockPulseRepository.Setup(pr => pr.GetAllPulsesAsync()).ReturnsAsync(pulses);

            // Act
            var result = await _pulseHandler.GetAllPulsesAsync();
        ...
        public async Task GetPulseAsync_PulseExists_ReturnsPulse()
        {
            ...
            _mockPulseRepository.Setup(pr => pr.GetPulseAsync(pulseId)).ReturnsAsync(pulse);

            // Act
            var result = await _pulseHandler.GetPulseAsync(pulseId);
        ...
        public void GetPulseAsync_PulseDoesNotExist_ThrowsMissingDataException()
        {
            ...
            _mockPulseRepository.Setup(pr => pr.GetPulseAsync(pulseId)).ReturnsAsync((Pulse?)null);

            // Act & Assert
            Assert.That(async () => await _pulseHandler.GetPulseAsync(pulseId),
